...
public interface IReleaseNotesRepository
{
    ...
    Task<ReleaseNoteDocument?> GetDocumentAsync(Guid id, Guid ownerUserId, CancellationToken cancellationToken);
    Task<IReadOnlyCollection<ReleaseNoteDocument>> GetLatestAsync(int count, Guid ownerUserId, CancellationToken cancellationToken);
}
